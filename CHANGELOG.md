...
- **Detecção de Bot e Origem de Acesso**: Aprimorado o fluxo para detectar se a rota pública foi consultada por um *Usuário Logado*, por uma *Pessoa* (visitante humano deslogado) ou por um *Bot/Crawler* (regex de user-agent + `navigator.webdriver`), incluindo o User-Agent do dispositivo na notificação do Telegram.
- **Rate Limiting no Middleware**: Atualizado [proxy.ts](file:///home/hans/devworkspace/santorini/proxy.ts) para limitar consultas na rota pública de extrato a no máximo 5 CPFs diferentes por minuto por IP (consultas repetidas ao mesmo CPF não contam). Excedido o limite, o IP recebe uma tela escura estilizada com HTTP status `429 Too Many Requests`.
- **Disjuntor de Emergência (Circuit Breaker)**: Criado o arquivo [security.ts](file:///home/hans/devworkspace/santorini/convex/security.ts) e a tabela `securityLogs`. Se o rate limiter bloquear mais de 3 IPs distintos nos últimos 5 minutos, a funcionalidade de extrato público é **desativada automaticamente** no Convex, um alerta crítico de segurança é enviado ao Telegram e novas requisições de extrato recebem imediatamente uma tela estilizada de erro `503 Service Unavailable`.
- **Feature Flag de Controle**: Registrada a chave `public_extratos_ativo` em [settings.ts](file:///home/hans/devworkspace/santorini/convex/settings.ts) permitindo que o administrador reative a funcionalidade pelo painel administrativo global.
