...
| DP-015 | A verificação de inadimplência adota a lógica de Saldo Acumulado (Expected vs Paid) com mensalidade padrão de R$ 50,00 a partir da data de adesão (joinedAt). | Implementada | Evita falsos alertas de inadimplência para associados que realizaram pagamentos adiantados cobrindo múltiplos meses em uma única transação. |
...
As decisões DP-001 e DP-002 estão detalhadas em [Modelo de negócio SaaS](modelo-negocio.md). As decisões DP-007, DP-008 e DP-009 estão especificadas em [Feedback Comunitário](feedback-comunitario.md). As decisões DP-003 e DP-005 estão registradas em [Identidade visual](identidade-visual.md) e no [Roadmap](roadmap.md). A decisão DP-011 está detalhada em [Tutoriais do usuário](tutoriais-usuario.md) e conectada ao plano de evolução do [Portal do Associado](portal-do-associado.md). As decisões DP-012, DP-013 e DP-014 estão consolidadas em [UX Mobile First, Ajuda e Splash](ux-mobile-first-ajuda-splash.md) e no [Roadmap](roadmap.md). A decisão DP-015 está descrita no plano de inadimplência.
